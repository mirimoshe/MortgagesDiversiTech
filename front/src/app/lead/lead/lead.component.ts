...
export class LeadComponent implements OnInit, AfterViewInit {
        ...
        if (response === '200') {
          ...
          this.userService.createUserForLead(this.user, this.lead_id!).subscribe({
            next: (createdUser: IUser) => {
              sessionStorage.setItem("user-id",`${createdUser.id}`);
              this.rrr=createdUser.id;
              ...
              //כנ"ל 
              console.log("createdUser 222", createdUser);
  ...
  convertToISO8601WithTime(date: string | Date | undefined): string {
    if (typeof date == 'string') {
      // נתח את המחרוזת לפורמט Date על בסיס פורמט 'yyyy-MM-dd'
      const parsedDate = parse(date, 'yyyy-MM-dd', new Date());

      // אם ניתוח התאריך נכשל, החזר מחרוזת ריקה
      if (isNaN(parsedDate.getTime())) {
        return '';
      }

      // קבע את השעה ל-00:00:00.000 UTC
      parsedDate.setUTCHours(0, 0, 0, 0);

      // צור את הפורמט עם אלפיות השנייה
      ...
      // אם התאריך הוא undefined, החזר מחרוזת ריקה
      return '';
    }
  }
  ...
  save() {
    ...
    try {
      ...
      console.log("111", this.customerData.userId);
      const user = {
        userName: this.firstFormGroup.value.userName,
        password: this.firstFormGroup.value.password,
        email: this.secondFormGroup.value.userName,
        role: Role.Customer,
        created_at: new Date(),
        updated_at: new Date()
      }
      // this.userService.createUserForLead(this.user, Number(this.lead_id)).subscribe({
      //   next: (res) => {
      //     const userId = res.id
      //     console.log("444", userId, typeof (userId));
      this.leadService.getLeadById(this.lead_id!).subscribe((lead) => {
        const yyy: ICustomer = {
          ...this.thirdFormGroup.value,
          ...this.fourthFormGroup.value,
          property_city: this.thirdFormGroup.value.property_city,
          transaction_type: this.thirdFormGroup.value.transaction_type,
          estimated_price_by_customer: +this.thirdFormGroup.value.estimated_price_by_customer,
          estimated_price_by_sales_agreement: +Number(this.thirdFormGroup.value.estimated_price_by_sales_agreement),
          has_other_properties: this.thirdFormGroup.value.has_other_properties,
          amount_of_loan_requested: +this.thirdFormGroup.value.amount_of_loan_requested,
          last_Name: this.secondFormGroup.value.last_Name,
          first_Name: lead.first_Name,
          email: this.firstFormGroup.value.userName,
          phone: lead.phone,
          connection: this.secondFormGroup.value.connection,
          t_z: this.secondFormGroup.value.t_z,
          birthDate: this.convertToISO8601WithTime(this.secondFormGroup.value.birthDate),
          family_status: this.secondFormGroup.value.family_status,
          number_of_people_in_house: +this.secondFormGroup.value.number_of_people_in_house,
          address: this.secondFormGroup.value.address,
          job_status: this.secondFormGroup.value.job_status,
          customer_type: 0,
          work_business_name: this.secondFormGroup.value.work_business_name,
          job_description: this.secondFormGroup.value.job_description,
          avarage_monthly_salary: +this.secondFormGroup.value.avarage_monthly_salary,
          years_in_current_position: +this.secondFormGroup.value.years_in_current_position,
          income_rent: +this.secondFormGroup.value.income_rent,
          income_Government_Endorsement: +this.secondFormGroup.value.income_Government_Endorsement,
          income_other: +this.secondFormGroup.value.income_other,
          expenses_rent: +this.secondFormGroup.value.expenses_rent,
          expenses_loans: +this.secondFormGroup.value.expenses_loans,
          expenses_other: +this.secondFormGroup.value.expenses_other,
          lead_id: this.lead_id,
          userId: this.rrr,
          created_at: new Date(),
          updated_at: new Date(),
        ...
        console.log("88",this.rrr);
        console.log("user 333", user);
        console.log("miri", yyy);
        this.customerService.updateCustomer(this.customerId, yyy).subscribe({
          next: (res) => {
            console.log("555", res);
